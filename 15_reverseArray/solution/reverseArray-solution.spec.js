const reverseArray = require('./reverseArray-solution');
...
describe("Reverse Array Elements", () => {
    ...
    test("apple, banana, cherry is return cherry, banana, apple", () => {
        expect(reverseArray(arr2)).toStrictEqual(result2);
    });
    test("true, false, true, false is return false, true, false, true", () => {
        expect(reverseArray(arr3)).toStrictEqual(result3);
    });
    test("10, 20, 30, 40, 50, 60 is return 60, 50, 40, 30, 20, 10", () => {
        expect(reverseArray(arr4)).toStrictEqual(result4);
    });
    test("empty is return empty", () => {
        expect(reverseArray(arr5)).toStrictEqual(result5);
    });
    test("hello is return hello", () => {
        expect(reverseArray(arr6)).toStrictEqual(result6);
    });
    test("a, b, c, d, e is return e, d, c, b, a", () => {
        expect(reverseArray(arr7)).toStrictEqual(result7);
    });
    test("3.14, 2.71, 1.618 is return 1.618, 2.71, 3.14", () => {
        expect(reverseArray(arr8)).toStrictEqual(result8);
    });
    test("true, hello, 42 is return 42, hello, true", () => {
